...
export default function App() {
  ...
  const [emailIntent, setEmailIntent] = useState<'save' | 'signin'>('save');
  ...
  return (
    <main className="game-shell">
      ...
      {showProfile && <ProfileModal profile={profile} stats={stats} email={emailDraft} setEmail={setEmailDraft} code={emailCode} setCode={setEmailCode} notice={emailNotice} emailIntent={emailIntent} onClose={() => setShowProfile(false)} onSave={storeProfile} onEmail={async () => { if (!onlineApi) { setEmailNotice('Email save becomes available after Supabase is connected.'); return; } try { setEmailIntent('save'); await onlineApi.sendEmailCode(emailDraft); setEmailNotice('Check your inbox for the six-digit code.'); } catch (error) { setEmailNotice(error instanceof Error ? error.message : 'The email code could not be sent.'); } }} onSignIn={async () => { if (!onlineApi) { setEmailNotice('Email sign-in becomes available after Supabase is connected.'); return; } try { setEmailIntent('signin'); await onlineApi.sendSignInCode(emailDraft); setEmailNotice('Check your inbox for the six-digit sign-in code.'); } catch (error) { setEmailNotice(error instanceof Error ? error.message : 'The sign-in code could not be sent.'); } }} onVerify={async () => { if (!onlineApi) { setEmailNotice('Email sign-in becomes available after Supabase is connected.'); return; } try { if (emailIntent === 'signin') { await onlineApi.verifySignInCode(emailDraft, emailCode); setEmailNotice('Signed in. This account keeps its own saved stats.'); } else { await onlineApi.verifyEmailCode(emailDraft, emailCode); setEmailNotice('Profile saved. This guest identity can now be recovered with your email.'); } } catch (error) { setEmailNotice(error instanceof Error ? error.message : 'That email code could not be verified.'); } }} />}
    </main>
  );
}
