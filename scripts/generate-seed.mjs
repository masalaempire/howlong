...
const sql = source.map((question) => {
  ...
  return `('${esc(question.id)}','${esc(question.category)}','${esc(question.prompt)}','${esc(question.hint)}',${answer},${min},${max},'${esc(question.displayAnswer)}','${esc(question.fact)}','${esc(sourceLabel(question.sourceUrl))}','${esc(question.sourceUrl)}','2026-09-18',${question.difficulty},'active')`;
}).join(',\n');
